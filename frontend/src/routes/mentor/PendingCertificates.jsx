...
export default function PendingCertificates() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["pendingCertificatesMentor", page, academicYear],
    queryFn: async () => {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/certificates/pending/mentor?page=${page}&limit=10&academicYear=${academicYear}`, {
        credentials: "include"
      });
      return res.json();
    }
  ...
  const handleApprove = async (certificateId, points) => {
    try {
      ...
      if (res.ok) {
        ...
        refetch();
        queryClient.invalidateQueries(["pendingCertificatesMentor"]);
      } else {
