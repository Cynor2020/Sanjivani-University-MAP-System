...
export default function PendingCertificates() {
  ...
  // Fetch department info to get years
  const { data: departmentData } = useQuery({
    queryKey: ["departmentInfo"],
    queryFn: async () => {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/users/me`, { credentials: "include" });
      return res.json();
    }
  });

  useEffect(() => {
    if (departmentData?.user?.department?.years) {
      setDepartmentYears(departmentData.user.department.years);
    }
  }, [departmentData]);

  const { data, isLoading, refetch } = useQuery({
    queryKey: ["pendingCertificates", sortBy, fromDate, toDate, departmentYear],
    queryFn: async () => {
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            ...
            {(sortBy !== "newest" || fromDate || toDate || departmentYear) && (
