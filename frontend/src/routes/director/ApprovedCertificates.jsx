...
export default function ApprovedCertificates() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["approvedCertificates", sortBy, fromDate, toDate, department, category, departmentYear, search],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/certificates/approved?${params}`,
        { credentials: "include" }
      );
      return res.json();
    }
  ...
  const deleteMutation = useMutation({
    mutationFn: async (certificateId) => {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/certificates/${certificateId}`,
        {
          method: "DELETE",
          credentials: "include"
        }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["approvedCertificates"]);
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Approved Certificates</h1>
        <p className="text-gray-600 mt-2">View and manage approved student certificates from all departments</p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Student name or PRN"
                className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 ${search ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-300'}`}
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 ${category ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-300'}`}
                >
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Approved Certificates ({data?.certificates?.length || 0})
            {(sortBy !== "newest" || fromDate || toDate || department || category || departmentYear || search) && (
        ...
        <CardContent>
          <div className="space-y-4">
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Approved: {new Date(cert.approvedAt).toLocaleDateString()} at {new Date(cert.approvedAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} 
                      by {cert.approvedBy?.name || "Unknown"}
                    </p>
                    {cert.cloudinaryUrl && (
                      <a
                  ...
                  <div className="flex space-x-2 ml-4">
                    <Button
                      ...
                      variant="destructive"
                      onClick={() => {
                        if (window.confirm("Are you sure you want to delete this approved certificate? This will remove the points from the student's total.")) {
            ...
            {(!data?.certificates || data.certificates.length === 0) && (
              <div className="text-center py-8 text-gray-500">No approved certificates</div>
            )}
          </div>
        </CardContent>
      </Card>
